...
resource "azurerm_resource_group" "rg-pub" {
  name     = "example-resources-pub"
  location = "australiaeast"
}

resource "azurerm_public_ip" "ippub" {
  name                = "acceptanceTestPublicIp1"
  resource_group_name = azurerm_resource_group.rg-pub.name
  location            = azurerm_resource_group.rg-pub.location
